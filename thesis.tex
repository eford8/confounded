...
\usepackage{tocloft}
\usepackage[bookmarksnumbered,bookmarksopen=true,bookmarks=true,hidelinks,pdfpagelabels=true]{hyperref}
\usepackage{bookmark}

% Make all figures and tables hang towards the top
\makeatletter
...
% Prevent captions from setting stuff 2x and therefore making 2 pdfbookmarks
\usepackage{caption}
\captionsetup{singlelinecheck=off}

% Some manual \pdfbookmarks are too low. This should raise them.
\makeatletter
...
\begin{titlingpage}
	...
	\vfill
	\noindent
	Keywords:\@ batch effects, batch correction, gene expression, transcriptomics, deep learning, adversarial neural network, variational autoencoder % the \@ removes the double "\nonfrenchspacing" spaces after the colon, which is required for thesis submission
\end{titlingpage}
...
\newpage
\phantomsection
\addcontentsline{toc}{section}{List of \tablename{s}}
{% Change table refs "1", "2" -> "Table 1", "Table 2" in LOT
\let\oldnumberline\numberline%
...
\newpage
\phantomsection
\addcontentsline{toc}{section}{List of \figurename{s}}
{% See comment on list of tables
\let\oldnumberline\numberline%
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:workflow}: Batch adjustment justification and steps}{fig:workflow}
	\centering
	% TODO I changed the width so "Figures" would be on the same page as the 1st fig per Gentri's request. Change this back eventually
	\includegraphics[width=0.85\columnwidth]{figures/final/adjuster_workflow.pdf}
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:network}: Network architecture of Confounded}{fig:network}
	\centering
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:training_loss}: Autoencoder and discriminator loss}{fig:training_loss}
	\centering
...
% \paragraph{Synthetic Batch Effects} % TODO: uncomment this after submitting thesis
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:pca}: Principal components analysis (PCA)}{fig:pca}
	\centering
...
\end{figure}
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:tsne}: T-distributed Stochastic Neighbor Embedding (t-SNE)}{fig:tsne}
	\centering
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:mse}: Mean squared error (MSE)}{fig:mse}
	\centering
...
\begin{table}
	...
	\caption[Mean squared error (MSE)]{
		\pdfbookmarkraised{1}{Table \ref{tab:mse}: Mean squared error (MSE)}{tab:mse}
...
\end{table}
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:mmd}: Maximum mean discrepancy (MMD)}{fig:mmd}
	\centering
...
\begin{table}
	...
	\caption[Maximum mean discrepancy (MMD)]{
		\pdfbookmarkraised{1}{Table \ref{tab:mmd}: Maximum mean discrepancy (MMD)}{tab:mmd}
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:batch}: Batch classification accuracy}{fig:batch}
	\centering
...
\begin{table}
	...
	\caption[Batch classification accuracy]{
		\pdfbookmarkraised{1}{Table \ref{tab:batch}: Batch classification accuracy}{tab:batch}
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:true_class}: True class classification accuracy}{fig:true_class}
	\centering
...
\begin{table}
	...
	\caption[True class classification accuracy]{
		\pdfbookmarkraised{1}{Table \ref{tab:true_class}: True class classification accuracy}{tab:true_class}
...
\begin{figure}
	\pdfbookmarkraised{1}{\figurename{} \ref{fig:mnist}: MNIST handwritten digits}{fig:mnist}
	\centering

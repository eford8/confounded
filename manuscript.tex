...
%	and there may be systemic effects when integrating multiple datasets.
	...
	%I'm not sure if we need to put my repository on there since it's different than jonathan's now
\end{abstract}
...
Because of the sensitive nature of technologies used to generate these data (i.e., expression levels may respond drastically to small environmental changes), expression data often contain unwanted confounding effects that can skew analyses. % removed an s from contains
Three examples of these include batch effects \label{it:batch}, known covariates, and dataset-integration effects. %included and
%kind of seems like we say for example/ and example many times. 
